...
const otpFormSchema = z.object({
  otp: z
    ...
    .regex(new RegExp(REGEXP_ONLY_DIGITS), 'OTP must contain only digits.'),
});

const newPasswordFormSchema = z.object({
  password: z
    .string()
    .min(12, 'Password must be between 12 and 64 characters long.')
    .max(64, 'Password must be between 12 and 64 characters long.'),
});
...
export function PasswordResetForm({
  ...
  const [verificationCodeDialogOpen, setVerificationCodeDialogOpen] =
    useState(false);
  ...
  const onEmailSubmit = () => {
    ...
    const adminEmailExists = admins.some(
      (admin) => admin.email === formInputAdminEmail,
    );
    ...
    <div className={cn('flex flex-col gap-6', className)} {...props}>
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Enter your email below associated with your account to send password
            reset code
          </CardDescription>
        </CardHeader>
        <CardContent>
          ...
          >
            <DialogContent className="w-[380px] max-w-full h-[260px] max-h-[90vh]">
              <div className="flex flex-col items-center justify-center text-center gap-4">
                ...
                <Form {...otpForm}>
                  <form
                    onSubmit={otpForm.handleSubmit(onVerificationCodeSubmit)}
                    className="space-y-6"
                  >
                    <FormField
